...
const determineIfUserInOntario = (postalCode) => {
  if (ONTARIO_POSTAL_CODE_FIRST_LETTER.includes(postalCode.charAt(0).toUpperCase())) {
    return true
  }
  return false
}
...
const getNewCasesActiveSubscribers = async () => {
  var subscribers = []
  ...
  const [rows] = await connection.execute('Select * From NewCasesSubscribers where active=1')

  rows.forEach((row) => {
    subscribers = [...subscribers, { Subscriber: row.Subscriber, PostalCode: row.PostalCode }]
  })
